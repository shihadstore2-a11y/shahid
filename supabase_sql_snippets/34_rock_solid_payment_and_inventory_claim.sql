-- ===================================================================
-- 34: Rock-Solid Universal Payment Confirmation, Auto-Claim & RLS Fix
-- ===================================================================
-- الهدف:
-- 1. دالة ذرية وشاملة (process_successful_payment) تقوم بتأكيد الدفع وصرف المخزون
--    وتحويل حالة الطلب إلى (fulfilled 🎁) أو (paid ✅) فوراً
-- 2. منع بقاء أي طلب مدفوع في حالة (السلات المتروكة)
-- 3. تصحيح سياسات RLS بالكامل لتمكين قراءة وتحديث الطلبات دون قيود
-- ===================================================================

-- 1. تحويل نوع المزود في جدول المخزون ليدعم أي اسم باقة أو مزود
DO $$
BEGIN
...
BEGIN
  ...
  -- إذا كان مسلماً مسبقاً وتوجد بيانات اشتراك
  ...
  -- ب. مطابقة مباشرة بالمدة والمزود
  ...
  -- هـ. إذا وُجد حساب متاح في المخزون -> صرف فوري وتسليم
  ...
  -- و. في حال عدم توفر مخزون -> تثبيت الطلب كـ (paid ✅) بانتظار التسليم اليدوي
...
-- 4. الدالة الرئيسية الشاملة لتأكيد الدفع ومعالجة الطلب (process_successful_payment)
CREATE OR REPLACE FUNCTION public.process_successful_payment(_order_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
DECLARE
  ...
  v_final_status TEXT;
BEGIN
  -- أ. تحديث حالة الطلب إلى paid مباشرة
  UPDATE public.orders
  SET status = 'paid', updated_at = NOW()
  ...
  -- في حال كان الطلب قد تم تأكيده مسبقاً
  ...
  -- ب. تحديث معاملة الدفع
  UPDATE public.payment_transactions
  SET status = 'success', updated_at = NOW()
  WHERE order_id = _order_id;

  -- ج. محاولة صرف الحساب من المخزون فوراً
  BEGIN
...
-- 5. إبقاء confirm_order_paid كـ alias يوجه إلى process_successful_payment
...
AS $$
BEGIN
  RETURN public.process_successful_payment(_order_id);
END;
$$;


-- 6. دالة عرض تفاصيل الطلب والتسليم الآمنة (get_order_delivery_status)
...
-- 7. منح الصلاحيات لكافة الدوال
REVOKE ALL ON FUNCTION public.process_successful_payment(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.process_successful_payment(UUID) TO anon, authenticated, service_role;
...
-- 8. سياسات RLS للطلبات لضمان عدم حجب أي طلب
...
-- 9. تحديث كاش Schema لـ PostgREST
